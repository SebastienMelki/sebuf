...
For JSON requests and responses, the client automatically uses custom sebuf JSON marshalers when they are present on your protobuf types. These package-level marshalers are generated by `protoc-gen-go-http`; generate `go-http` alongside `go-client` when you need raw `json.Marshal`/`json.Unmarshal` or client calls to honor sebuf JSON-mapping annotations such as `unwrap`, `int64_encoding`, `enum_encoding`, `nullable`, `empty_behavior`, `timestamp_format`, `bytes_encoding`, `oneof_config`, or `flatten`. See [JSON/Protobuf Compatibility](./json-protobuf-compatibility.md) for details.
