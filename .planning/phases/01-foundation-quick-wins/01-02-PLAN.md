...
- Closing comment documents the `(sebuf.http.unwrap) = true` syntax with proto example
- Closing comment shows expected JSON output (`[{...}, {...}]` vs `{"items": [...]}`)
- Closing comment references test files (`unwrap.proto`, `unwrap.go`, `unwrap_test.go`)
  </done>
</task>
...
- Closing comment adequately documents proto3 `json_name` field option as the existing solution
- Comment includes proto example showing per-field overrides and references protobuf.dev docs
  </done>
</task>
