...
must_haves:
  ...
  key_links:
    - from: "internal/clientgen/generator.go"
      ...
      pattern: "annotations\\.GetMethodHTTPConfig|annotations\\.GetServiceHeaders|annotations\\.GetQueryParams"
...
BUT: verify that `annotations.BuildHTTPPath` produces identical output. The inline version does NOT add a leading slash if both are empty (it would produce empty string), while `BuildHTTPPath` returns "/" for both empty. Check if this case can actually occur -- if `httpPath` comes from `httpConfig.Path` and `basePath` from service config, both empty means no annotation, which is handled earlier. If confident, replace. If not, keep inline logic for safety.
...
If golden file tests exist for clientgen, run them and verify ZERO golden file changes.
  ...
  <done>clientgen/annotations.go (241 lines) deleted. clientgen/generator.go imports and uses shared annotations. All clientgen tests pass with zero behavior change.</done>
</task>
...
- Replace inline path building (lines ~281-288) with `annotations.BuildHTTPPath(basePath, httpPath)` -- same caveat as clientgen about empty path case.
