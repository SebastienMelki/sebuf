...
// emitInlineUnmarshalChild emits the inline dispatch that forwards opts to a child's
// UnmarshalJSONSebuf when available and falls back to opts.Unmarshal otherwise. The
// dispatch is inlined for the same reason as emitInlineMarshalChild: two proto files
// in the same Go package can both produce unwrap files without redeclaring a helper.
// Output decodes rawExpr into the message bound to targetExpr and returns on error.
func emitInlineUnmarshalChild(gf *protogen.GeneratedFile, rawExpr, targetExpr string) {
	gf.P(
		"if u, ok := any(",
		targetExpr,
		").(interface{ UnmarshalJSONSebuf([]byte, protojson.UnmarshalOptions) error }); ok {",
	)
	gf.P("if err := u.UnmarshalJSONSebuf(", rawExpr, ", opts); err != nil {")
	gf.P("return err")
	gf.P("}")
	gf.P("} else if err := opts.Unmarshal(", rawExpr, ", ", targetExpr, "); err != nil {")
	gf.P("return err")
	gf.P("}")
}
...
func (g *Generator) generateUnwrapMapUnmarshal(
	...
	if elementTypeIdent != nil {
		...
		emitInlineUnmarshalChild(gf, "itemRaw", "item")
...
func (g *Generator) generateRootMapMessageValueUnmarshal(
	...
	emitInlineUnmarshalChild(gf, "v", "item")
