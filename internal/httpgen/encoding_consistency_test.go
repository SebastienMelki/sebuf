...
// produce identical int64 MarshalJSON code.
...
// extractMarshalJSON extracts all MarshalJSON function blocks from Go source code.
// This is used by consistency tests to compare the serialization logic between
// go-http and go-client generators while ignoring the intentional UnmarshalJSON
// divergence (go-client uses UnmarshalJSONSebuf with opts forwarding).
func extractMarshalJSON(content string) string {
	...
	for _, line := range lines {
		if !inMarshal && strings.Contains(line, "MarshalJSON") &&
			!strings.Contains(line, "Unmarshal") {
			inMarshal = true
			braceDepth = 0
		}
...
// compareEncodingFiles compares go-http and go-client encoding golden files.
// MarshalJSON must be byte-identical (serialization correctness).
// UnmarshalJSON is allowed to differ because go-client uses UnmarshalJSONSebuf
// with opts forwarding for forward-compatibility.
func compareEncodingFiles(t *testing.T, httpgenFile, clientgenFile, label string) {
	t.Helper()

	httpgenContent, httpErr := os.ReadFile(httpgenFile)
	if httpErr != nil {
		t.Fatalf("Failed to read httpgen %s golden file: %v", label, httpErr)
	}

	clientgenContent, clientErr := os.ReadFile(clientgenFile)
	if clientErr != nil {
		t.Fatalf("Failed to read clientgen %s golden file: %v", label, clientErr)
	}

	httpMarshal := extractMarshalJSON(string(httpgenContent))
	clientMarshal := extractMarshalJSON(string(clientgenContent))

	if httpMarshal != clientMarshal {
		t.Errorf("go-http and go-client %s MarshalJSON code differs", label)
		t.Logf("First difference:\n%s", findFirstDifference(httpMarshal, clientMarshal))
	} else {
		t.Logf("go-http and go-client produce identical %s MarshalJSON code", label)
	}
}
