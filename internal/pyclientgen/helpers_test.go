...
func TestIsInvalidIdentifier(t *testing.T) {
	tests := []struct {
		input string
		want  bool
	}{
		// Empty is invalid: cannot be an identifier.
		{"", true},
		// Leading digit makes it invalid.
		{"1A", true},
		{"9_thing", true},
		// Underscore is fine as a leading char.
		{"_x", false},
		// Plain word is fine.
		{"x", false},
		{"thing", false},
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			got := isInvalidIdentifier(tt.input)
			if got != tt.want {
				t.Errorf("isInvalidIdentifier(%q) = %v, want %v", tt.input, got, tt.want)
			}
		})
	}
}
